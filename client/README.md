...
This repository contains the client-side code for Fishoria. It combines a custom WebGL game engine (built on PixiJS) with a modern React UI to deliver high-fidelity fishing gameplay. The architecture separates the game engine (rendering, physics, fish behavior) from the application layer (state, routing, API), enabling both to evolve independently.
...
- **Advanced Fish AI** – steering behaviors, depth preferences, bait attraction, and species-specific activity patterns
